...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, if_top=True):
        self.inplanes = 64
        self.if_top = if_top
...
def resnet50_nofc(pretrained=False, **kwargs):
    """Constructs a ResNet-50 model without fc layer.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet(Bottleneck, [3, 4, 6, 3], 1000, False)
    if pretrained:
        model.load_state_dict(remove_fc(model_zoo.load_url(model_urls['resnet50'])))
    return model

def resnet50_nofc_pre(pretrained=False, **kwargs):
    """Constructs a ResNet-50 model without fc layer.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNetpre(Bottleneck, [3, 4, 6, 3], 1000, False)
    if pretrained:
        model.load_state_dict(remove_fc(model_zoo.load_url(model_urls['resnet50'])))
    return model
