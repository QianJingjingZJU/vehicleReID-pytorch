    ...
    for epoch in range(num_epochs):
        ...
        for batch in range(batchnumber):
            ...
            if (batch+1)%30 == 0:
                ...
                print('Epoch [{}] Batch [{}] Loss: {:.4f} SoftmaxLoss: {:.4f} TriphardLoss:{:.4f} Time: {:.4f}s'. \
                        format(epoch, batch+1, batch_loss,batch_softmaxloss, batch_triphardloss, time.time()-begin_time))
                begin_time = time.time()
        ...
        print('Loss: {:.4f} SoftmaxLoss: {:.4f} TriphardLoss: {:.4f}'.format(epoch_loss,epoch_softmaxloss,epoch_triphardloss))
